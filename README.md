...
POST /notifications сразу возвращает уведомление со статусом pending, а сама отправка через нужный канал идёт в фоне. После попытки статус обновляется на sent или failed, актуальное состояние можно посмотреть через GET /notifications/{id}.
